...
def create_vlan(iface, vlan, ipv4_addr=None, ipv6_addr=None):
    try:
    ...
    if ipv4_addr:
        ip_addr_mgmt["type"] = "static"
        ip_addr_mgmt.setdefault("addresses", [])
        ip_addr_mgmt["addresses"].append({"address": ipv4_addr})
    if ipv6_addr:
        ip_addr_mgmt["type"] = "static"
        ip_addr_mgmt.setdefault("addresses", [])
        ip_addr_mgmt["addresses"].append({"address": ipv6_addr})
...
def create_deployment(
    ...
    if node_iface and vlan:
        create_vlan(node_iface, vlan, ipv4_addr, ipv6_addr)
